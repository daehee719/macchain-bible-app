...
export const Alert: React.FC<AlertProps> = ({
  ...
  return (
    <div
      className={`border rounded-lg p-4 mb-4 flex justify-between items-start ${variantStyles[variant]} ${className || ''}`}
      role="alert"
    >
      <div className="flex-1">
        {title && (
          <h3 className={`font-medium mb-1 ${titleStyles[variant]}`}>{title}</h3>
        )}
      ...
      {onClose && (
        <button
          onClick={onClose}
          className="ml-4 text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 transition-colors"
